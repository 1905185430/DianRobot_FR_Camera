...
from kyle_robot_toolbox.open3d import * # type: ignore
...
class ArucoPosePublisher(Node):
    def __init__(self):
        ...
        # 创建ArucoTag检测器（使用相对路径，基于脚本位置）
        base_dir = Path(__file__).resolve().parent
        arucotag_dir = base_dir / "arucotag"
        config_path = str(arucotag_dir / "arucotag.yaml")
        self.arucotag = ArucoTag(self.camera, config_path=config_path)
        # 创建ArucoTag可视化窗口
        ...
        # 配置视角
        json_path = str(arucotag_dir / "render_option.json")
        ...
        while rclpy.ok():
            try:
                ...
                if has_aruco:
                    ...
                    aruco_centers_filter = aruco_centers[valid_aruco_mask] # pyright: ignore[reportIndexIssue]
                    aruco_corners_filter = aruco_corners[valid_aruco_mask] # pyright: ignore[reportIndexIssue]
                    T_cam2aruco_by2d_filter = T_cam2aruco_by2d[valid_aruco_mask] # pyright: ignore[reportIndexIssue]
