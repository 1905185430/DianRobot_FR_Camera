...
tag_trans_mat = []
fr5_A = []
robot = Robot.RPC('192.168.58.2')  # 替换为实际的机器人IP地址
...
def init():
    ...
    time.sleep(1)
    robot.MoveGripper(1, 100, 50, 30, 10000, 1, 0, 0, 0, 0)
    input('------等待按下回车夹住------')
    robot.MoveGripper(1, 72, 50, 30, 10000, 1, 0, 0, 0, 0)
    time.sleep(0.5)

def save_to_file(matrix):
    from pathlib import Path
    # 保存到项目内的 data/calibration 目录（相对于本脚本），文件使用时间戳命名
    base_dir = Path(__file__).resolve().parent.parent
    log_dir = base_dir / 'data' / 'calibration'
    log_dir.mkdir(parents=True, exist_ok=True)

    timestamp = time.strftime('%Y%m%d_%H%M%S')
    file_name = f'camera2base_{timestamp}.json'
    file_path = log_dir / file_name

    matrix_list = matrix.tolist()
    with file_path.open('w', encoding='utf-8') as file:
        json.dump(matrix_list, file, ensure_ascii=False, indent=2)
...
def main(args=None):
    ...
    for i in range(sample_times):
        ...
        fr5_A_end = robot.GetActualToolFlangePose(0)
        # Normalize return value: some APIs return (status, pose_list), others return pose_list directly.
        if isinstance(fr5_A_end, tuple) and len(fr5_A_end) >= 2 and isinstance(fr5_A_end[1], (list, tuple)):
            pose = list(fr5_A_end[1])[-6:]
        elif isinstance(fr5_A_end, (list, tuple)):
            pose = list(fr5_A_end)[-6:]
        else:
            raise RuntimeError(f"Unexpected return from GetActualToolFlangePose: {fr5_A_end}")
        end2base = get_transform_mat(pose[0], pose[1], pose[2], pose[3], pose[4], pose[5])
